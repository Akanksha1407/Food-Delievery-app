  ...
  <div className="container-fluid">
    ...
    <div className="collapse navbar-collapse" id="navbarNav">
      <ul className="navbar-nav me-auto mb-2">
        ...
        ?<li className="nav-item">
              <Link className="nav-link active fs-5 mx-3 fw-bold" aria-current="page" to="/myOrderData">My Orders</Link>
          </li>
        :""
        }
      </ul>
      ...
      :
      <div>
        <div className='btn bg-white text-danger fw-bold mx-2' onClick={()=>{setCartView(true)}}>
          My Cart {" "}
          <Badge pill bg="success">{data.length}</Badge>
        </div>
        {cartView? <Modal onClose={()=>{setCartView(false)}}><Cart/></Modal>: null}

        <div className='btn bg-white text-success fw-bold mx-2' onClick={handleLogout}>
          Logout
        </div>
      </div>  
      }  
